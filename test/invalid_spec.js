...
// TODO: use all of the following values in the tests accordingly
// this was prevoiusly done by the previous dev
// but the error messages have been changed and need updating
const errsType = {
...
describe('invalid data - form UI', () => {
  ...
  it('throws on error. default is 1 message, no translation', done => {
    ...
    const fcn = () => {
      validate.form(schema, joiOptions, undefined, true)(hookBad);
  ...
  // TODO: Understand and fix this test
  it('throws on error. translate by type', done => {
    ...
    const fcn = () => {
      validate.form(schema, joiOptions, translations, true)(hookBad);
      assert(false, 'validate.form callback unexpectedly called');
      done();
    };

    assert.throws(fcn, errors.BadRequest);
    done();
  });
  ...
  it('throws on error. translate using substrings', done => {
    const translations = [
      ...
      { regex: /required pattern/, message: '"${key}" is badly formed.' },
    ];
...
describe('invalid data - Mongoose', () => {
  ...
  it('throws on error. translate by type', done => {
    ...
    const fcn = () => {
      validate.mongoose(schema, joiOptions, translations, true)(hookBad);
